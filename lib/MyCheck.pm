...
sub check_cloudview_exsit {
    ...
    if ( scalar @file == 1 ) {
        ...
        croak
"Make sure cloudview software  is in ( cloudview_deploy ) directory !";
    }
}
...
sub check_cloudview_software {
    ...
    my @mysql = glob
      './cloudview/Supports/third-party_tools/installer_of_mysql64/install*.sh';
    my @hyper = glob
'./cloudview/Supports/third-party_tools/cvm-hypervisor-install/cvm-hypervisor-install-*/install';
    ...
    croak "hypervisor install  script miss in cloudview dir"
      unless scalar @hyper;
}
